...
2. 按 Issue 正文中的 `request_id` 分组，同一 request 的来源级 Issue 必须作为一个诊断批次；批次按最早 Issue 创建时间排序，每次最多处理 3 个 request 批次。没有待处理批次时静默结束。
3. 从同批次全部 Issue 正文中的 `repair_request.json` 读取 `request_id`、`base_commit_sha`、`health_report_sha256`、findings、约束和运行链接；确认 request 与 commit 一致后，只读相关代码、工作流、运行日志及报告证据。
...
9. 不论结论如何，都不得新增或编辑 Issue、评论或标签。用户批准后，Codex 才可使用本地已授权 GitHub CLI 向同一 request 的全部关联 Issue 回写方案并执行后续流程。
