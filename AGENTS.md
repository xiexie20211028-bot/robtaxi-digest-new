...
- **High**：共享筛选/评分/证据/摘要逻辑、P0 或多路线风险，在通用研发策略启用后允许自动实现，经可信业务回放与绑定当前 PR 提交的 Codex 独立复核后可自动合并。策略尚未启用时仍执行“批准执行”到 Draft PR、“批准合并上线”才合并的规则。
- **人类保留事项**：密钥与权限、重要数据删除、新增付费服务或超预算、生产路线切换、额外外部推送、自动化授权和验收门禁本身的修改，不能由模型互相批准。当前架构迁移以用户明确批准的实施方案和 #100 为依据；新授权只能在主分支启用验收通过后生效。
